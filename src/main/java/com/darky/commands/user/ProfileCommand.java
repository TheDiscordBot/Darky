...
public class ProfileCommand implements ICommand {
    @Override
    public void onCommand(CommandEvent event, Member member123, TextChannel channel, String[] args) throws Exception {
        Messages.sendMessage(event.getCache(), channel, "Please wait!", "Building the Message...", event.getAuthor()).queue( msg -> {
            try {
                ...
                calendar.setTimeInMillis(event.getCache().getUser(event.getAuthor()).getCreated());
                ...
                g.drawString(String.valueOf(event.getCache().getUser(event.getAuthor()).getCoins()), 320, 370);

                g.drawString("Miner", 100, 440);
                g.drawString(String.valueOf(event.getCache().getMinerFromUser(member.getUser().getIdLong()).size()), 320, 440);
