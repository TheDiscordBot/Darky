...
public class CooldownCommand extends AbstractCommand {
    ...
    public void onAdd(CommandEvent event, Member member, TextChannel channel, String[] args) {
        if (args.length==3) {
            ...
            sendMessage(event.getCache(), event.getChannel(), "Success!", "Added "+event.getMessage().getMentionedMembers().get(0).getUser().getName()+
    ...
    public void onremove(CommandEvent event, Member member, TextChannel channel, String[] args) {
        if (args.length==3) {
            ...
            sendMessage(event.getCache(), event.getChannel(), "Success!", "Removed "+event.getMessage().getMentionedMembers().get(0).getUser().getName()).queue();
        } else onHelp(event, member, channel, args);
    }
}
